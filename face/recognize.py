...
# Who counts as a visitor at all (2026-09-23). insightface keeps any box
# over 0.5, and a bag of clutter behind the booth table scored 0.69-0.72
# at 81-86 px wide, frame after frame -- steady enough to pass the 2 s
# "stable face" test, so the robot introduced itself to an empty room and
# then kept its gaze on the bag. A real person at the table, half turned
# away, scored 0.88 at 161-167 px (1920 px frame, ~0.9 m). Size is the
# gate: confidence cannot be, since the real face in the 640 px fixture
# clip (tests/fixtures/video) scores 0.71-0.74, the same as the bag. The
# score floor only drops junk. Someone sitting at the booth is well
# inside 1.3 m.
VISITOR_MIN_SCORE = 0.6
VISITOR_MIN_WIDTH = 0.06    # of the frame width: 115 px at 1920, ~1.3 m
...
def is_visitor(face: Face, frame_width: float) -> bool:
    """A confident, near enough face: someone at the booth, not a
    face-shaped thing in the background or a person across the room."""
    x1, _y1, x2, _y2 = face.bbox
    return (face.score >= VISITOR_MIN_SCORE
            and (x2 - x1) >= VISITOR_MIN_WIDTH * frame_width)


def visitors(faces: list, frame_width: float) -> list:
    """``faces`` with everything that is not a visitor dropped (order kept)."""
    return [f for f in faces if is_visitor(f, frame_width)]
...
def analyze_all(image) -> list:
    """Every face in the frame with its embedding, largest first (T17.3:
    the session's face may not be the largest one when somebody leans
    in from the next chair)."""
    faces = get_app().get(image)
    out = [Face(bbox=tuple(float(v) for v in f.bbox[:4]),
...
def detect_all(image) -> list:
    """Every face's bounding box, largest first, detector only."""
    app = get_app()
    bboxes, _kpss = app.det_model.detect(image, max_num=0, metric="default")
    if bboxes is None or len(bboxes) == 0:
        return []
    out = [Face(bbox=tuple(float(v) for v in b[:4]), score=float(b[4]))
           for b in bboxes]
    out.sort(key=lambda f: f.area, reverse=True)
    return out
