...
| T17 | Family feedback round 4: let them finish, trust a confirmed identity, one language policy, intake and closing | T15, T16 | in progress | 2026-09-05 |
...
**What the log showed.** A child's lesson was ended twice by the voice
...
a "welcome back"). The next visitor enrolled, a voice challenge downgraded
them to "is that you?", somebody said yes, and from then on the voice check
was off; when another family member took the seat the robot tutored him
under the first visitor's name for five minutes, because the face is never looked at again
...
## T17 — Family feedback round 4: let them finish, trust a confirmed identity, one language policy, a real intake and closing

**Goal.** What the fourth family session (2026-09-05, the first on T15
and T16, the first Russian-native visitor learning English) asked for,
with the log evidence in [progress/T17.md](progress/T17.md) and the
debrief the family recorded afterwards. Logs in
`booth/logs/2026-09-05_family/`.

**What the log showed.** The robot cut the mother off when she paused
to think, twice, and could not stop: cloud turn-taking is Gemini's
server VAD with no settings passed. "Dwayne" was voice-challenged five
times while her face scored 0.6–0.95, because the challenge cue never
consults the face, and the model then un-identified a confirmed
visitor from a `look` frame and a tease. A bystander's face was "the
largest" for 2–5 s about once a minute (34 `other` verdicts near 0),
so anyone leaning in for 3 s ends a lesson. The mother was enrolled
from one sentence with target, level, goal and native language
invented, then the lesson ping-ponged between Russian and English and
never corrected a mistake until she complained. A spoken
fill-in-the-blank had no audible blank and no options for four
repetitions. Real feedback was given three times and recorded once.
Three minutes of "turn your head, any other people?" were obeyed to
the letter.

**Subtasks.** Ordered by booth value; each has its own tests under
`tests/t17/` and its own line in the Progress log.

- **T17.1 — Let them finish.** Pass Gemini's VAD settings from the
  agent: `GeminiVADParams(end_sensitivity=LOW, silence_duration_ms=N,
  prefix_padding_ms=…)` through the service settings, `N` from
  `--turn-patience-ms` (default ~1800) and `BOOTH_TURN_PATIENCE_MS`;
  the briefing adds "when the student pauses mid-sentence, wait". The
  `turn:` median will rise by about the same amount; record it. Tests:
  the settings object carries the VAD config; live: a voice file with a
  1.2 s mid-sentence pause yields one transcription and one reply, not
  two.
- **T17.2 — Trust a confirmed identity; the voice asks once, and
  learns.** (a) No `challenge`/`downgrade` cue while the session's face
  was seen within `face_vouch_secs`; the mismatch is logged and
  overruled (the same rule T15.1 applies to "speaker changed"). (b)
  After a successful `confirm_identity`, or after the face has vouched
  twice, `VoiceIdentity.trust()` switches voice decisions off for the
  rest of the session (the family: "if they confirmed once or twice,
  act as that person"). (c) The samples heard around a confirmation
  are averaged into the profile's stored print, and the strengthened
  session face (T15.10) is written back to the profile on confirm (the
  family: "it should keep training its recognizers when the person
  confirms"). (d) Briefing: identity is the recognizer's call
  (`confirm_identity`), never a judgement from a `look` frame or from
  what the visitor says about their looks; after a confirmed yes the
  question is closed for the session. Tests: cue suppressed with the
  face present; trust after confirm; print and face updated; needles.
- **T17.3 — Bystanders do not count.** `recognize.embed_all(image)`
  returns every face; the runner's mid-session check compares the
  session face with all of them and reports `same` when any matches,
  whichever is largest; `other` only when no face matches; a swap
  needs a non-matching face *and* the session face absent for
  `swap_secs`. The tracker follows the matched face's box when one is
  known. Tests on the three-person fixtures: two faces with the
  session's smaller → same; a bystander alone → other; the swap clip
  still swaps.
- **T17.4 — Intake as a first meeting, enforced in code.** A
  per-session `Intake` the model fills one field per turn through
  `intake_answer(field, value)`: name, target language, explain-in
  (native / target / both), level, goal, minutes available, what they
  want today, correction preference (every mistake / only what blocks
  understanding / at the end). `enroll_new_learner` takes no fields
  and refuses with the list of what is still missing, so a one-sentence
  enrollment (the mother, 19:27:58) cannot happen. A returning learner
  gets the two session questions (minutes, today) in the greeting cue;
  `set_session_plan(minutes, focus)` stores them and the model says a
  two-or-three-step plan aloud. The runner injects a cue at 80 % of the
  agreed minutes ("wrap up, then the closing question"). When the
  student corrects a field aloud ("I'm advanced"), the matching tool
  must be called (needle). Tests: refusal lists missing fields; commit
  when complete; time cue fires; needles.
- **T17.5 — One language policy.** The explain-in choice from intake
  is stated in one line of the briefing and honoured: explanations
  and task-setting in the chosen language, practice in the target;
  when the student drifts into their own language, answer once in it
  and set the next task in the target and say so (19:35:03). Hard
  rules with needles: never ask them to say in the target something
  you just said in the target (19:30:10, 19:29:26); write every
  target-language word in its own script, never transliterated, so the
  voice reads it natively (19:13:43 "Zdravstvuyte"). Tests: briefing
  for ru/en with each policy; needles; the ru-native live row.
- **T17.6 — Spoken exercises by ear.** A `quiz(sentence_with_gap,
  options, answer, topic)` tool returns the exact script to read: the
  gap spoken as the native word for "blank", three options lettered,
  one question, and logs the exercise and the student's answer. The
  briefing allows only formats that work by ear: multiple choice,
  free response ("tell me in one sentence…"), role-play, repeat-after-
  me; before a grammar drill, one line naming the topic and an offer to
  explain it ("do you know what an adjective is?"). Tests: script
  format in ru and en; needles.
- **T17.7 — Corrections and the next rung, measured.** Briefing: after
  a student utterance in the target language with an error, repeat the
  corrected sentence once (a recast) and go on; when it is fine, offer
  one richer or more natural way to say it, at most once a turn; the
  intake's correction preference selects between these and
  "only what blocks understanding". An INFO `heard:` line logs the
  visitor's transcript beside `said:`. `tests/t17/judge_corrections.py`
  reads a session log and counts student errors vs corrections with a
  small Claude model (marked `anthropic`); run it on the 2026-09-05 log
  for the baseline (0 of ~6 in the mother's session) and after the
  change on the next session's log.
- **T17.8 — Fooling around: steer back with a budget.** A budget on
  sightseeing during a lesson: `look`, `move_head`, `turn_body` answer
  "skipped: enough sightseeing, back to the lesson" after two calls in
  two minutes (the T15.9 "skipped" pattern), so the model has to steer;
  the briefing: one playful answer to an off-topic request, then back
  to the language, a quip on the third ("I am a tutor, not a
  periscope"); if what they said makes no sense in context, say so and
  ask again rather than build on it; session notes are about the
  student's language, never about the robot. Tests: the budget counts
  and resets; needles.
- **T17.9 — The closing, recorded by code.** One closing question
  replaces the wish question: "what should I do better, or what would
  you want a robot like me to do?" (the family: feedback *and* ideas,
  one answer is fine). `record_wish` becomes `record_feedback(kind,
  text)`, and the runner also captures the visitor's next transcript
  after the closing cue into `booth/feedback.md` with date and name,
  tool call or not (three answers were given on 2026-09-05, one
  recorded). The "Go and practice" line waits for the answer. Tests:
  cue → transcript → file; needles.
- **T17.10 — Pace.** Gemini Live has no speaking-rate setting.
  Measure words per minute from `said:` text and the TTS stop
  timestamps already in the log (a `pace:` line per reply), try the
  prompt first (beginner: short sentences, slow, a pause after each),
  and if the number does not move, time-stretch the output audio by a
  per-level factor (WSOLA on `TTSAudioRawFrame`, pitch kept). Tests:
  the wpm line; the stretch processor keeps duration ratio and sample
  count.
- **T17.11 — Sweep.** The small things in progress/T17.md: the spoken
  tool call, `move_head` sides named from the visitor's view, the
  double greeting at a reconnect after a swap, the `heard:` line, the
  transcript-language noise.

**Definition of Done.** `tests/run.sh t17` green; the next family
session run through the protocol in progress/T17.md: nobody cut off
mid-sentence, no voice question with a matching face, a bystander
leaning in does not end a lesson, the mother's intake asks every
question, one language policy held for a session, three mistakes
recast, a spoken test with an audible blank and options, and every
closing answer in `booth/feedback.md`.

**Progress log.**
- 2026-09-05 — task written from the fourth family session's logs and
  the recorded debrief.
- 2026-09-05 (evening) — **T17.1–T17.11 built on the simulated path**
  (progress/T17.md "Built"): `tests/run.sh t17` 41 passed in the light
  venv, 48 in the voice venv; the whole unmarked suite 184 passed. Five
  older tests pinned briefing wording that changed (t3, t13, t15, t16)
  and were updated; the scripted live interviews (t9, t13, t14, t15)
  now answer all nine intake questions. Live: the cloud enrollment
  interview (`tests/t13 -k cloud`, eight scripted turns through
  `intake_answer`) passed; the seat-swap clip over the cloud voice
  (`tests/t15 -k seat_swap`) passed with the all-faces check and a
  one-breath intake; `tests/t17/probe_patience.py` measured one turn
  through a 1.2 s pause with the new VAD settings and two with
  Gemini's defaults; the corrections judge on the 2026-09-05 log gives
