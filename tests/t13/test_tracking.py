...
def test_steps_are_capped_and_within_limits():
    ...
    assert cmd["head_yaw"] <= math.radians(12) + 1e-9, "one step at most 12 deg"
    for i in range(30):
...
def test_body_takes_over_past_handoff():
    """head_yaw is the world-frame gaze: the base comes round under the
    head and the head keeps looking where it was (2026-09-23)."""
    robot = FakeRobot()
    t = FaceTracker(robot, clock=lambda: 0.0)
    now = 0.0
    body_moves = []
    ...
    first = body_moves[0]
    assert first["head_yaw"] > math.radians(35), "the gaze must not snap back"
    assert first["body_yaw"] == pytest.approx(first["head_yaw"])
    assert all(abs(s["head_yaw"]) <= HEAD_YAW_LIMIT for s in robot.sent)


def test_a_visitor_off_to_the_side_is_centred_and_stays_centred():
    """The 2026-09-23 lesson: a visitor ~40 deg to one side. The camera
    looks along the world-frame head_yaw, whatever the base does. The
    old handoff reset the head to 0 each time it passed 35 deg: the gaze
    swung back, the face went off-centre again, and the loop repeated
    every ~4.5 s with the body estimate climbing to 160 deg."""
    from tracking import pixel_to_angles, LITE_FOCAL_PER_WIDTH
    bearing = math.radians(40)
    fx = LITE_FOCAL_PER_WIDTH * W
    robot = FakeRobot()
    t = FaceTracker(robot, clock=lambda: 0.0)
    gaze = 0.0
    offsets = []
    now = 0.0
    for _ in range(60):                       # 30 s at 2 fps
        now += 0.5
        u = W / 2 - fx * math.tan(bearing - gaze)
        off = abs(pixel_to_angles(u, H / 2, W, H)[0])
        offsets.append(off)
        cmd = t.observe(box(u, H / 2), W, H, now=now)
        if cmd and "head_yaw" in cmd:
            gaze = cmd["head_yaw"]            # the move completes
    assert max(offsets[-30:]) <= math.radians(4.01), \
        "not centred after the first 15 s: %s" % offsets[-30:]
    assert abs(t.estimate["body_yaw"]) <= bearing + 1e-9
    later = [c for c in robot.sent[len(robot.sent) // 2:] if "head_yaw" in c]
    assert len(later) <= 2, "still chasing once centred: %s" % later
