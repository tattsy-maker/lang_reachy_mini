...
def test_a_learner_enrolled_mid_session_gets_their_own_briefing(tmp_path):
    """2026-09-23: Luca enrolled for Italian explained in Russian, but
    the stranger briefing ("greet them warmly in English") stayed the
    standing instruction and the whole lesson was in English. After an
    enrollment the runner reconnects with the learner's own briefing."""
    g = FakeGemini()
    store = LearnerStore(tmp_path / "learners")
    holder = CurrentLearner()
    ctx = FakeContext()
    runner = Runner(source="unused", store=store, holder=holder, context=ctx,
                    robot=None, brain=CloudBrain(g, ctx, ready_timeout=1.0))
    runner._recent_vectors = [unit(0)]

    async def visit():
        await runner.start_session(now=1.0)
        assert "do not recognize" in g._system_instruction_from_init
        holder.learner = store.create("Luca", "it", level="beginner",
                                      native_language="ru",
                                      explain_in="native")
        runner.schedule_rebrief("enrolled")
        await runner._rebrief_task

    asyncio.run(visit())
    briefing = g._system_instruction_from_init
    assert "do not recognize" not in briefing
    assert "Luca" in briefing and "Russian" in briefing
    assert g.calls.count("disconnect") == 2
    assert "Russian" in runner.cues[-1] and "do not greet" in runner.cues[-1]
