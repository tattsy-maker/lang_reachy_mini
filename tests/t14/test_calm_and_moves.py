...
def test_tracker_defaults_are_calm():
    ...
    # 2026-09-23: gain, dead-band and step retuned so the visitor ends up
    # centred; the rate limit and move length (what cured the twitch) hold.
    assert t.gain <= 0.65
    assert t.dead_band >= math.radians(3.9)
    assert t.min_interval >= 0.8
    assert t.max_step <= math.radians(12.01)
