...
# -- who counts as a visitor (2026-09-23) -----------------------------------

def test_visitor_gate_drops_the_background_bag_and_keeps_real_faces():
    """Measured boxes: a bag of clutter behind the booth table read as a
    face every frame and started sessions in an empty room; a person at
    the table half turned away; the fixture clip's real face, whose score
    is no better than the bag's (so size, not confidence, is the gate)."""
    from face.recognize import Face, is_visitor, visitors
    bag = Face(bbox=(1106, 450, 1192, 538), score=0.72)       # 1920 frame
    person = Face(bbox=(387, 363, 550, 560), score=0.88)      # 1920 frame
    far_person = Face(bbox=(900, 300, 990, 400), score=0.9)   # ~2 m away
    assert not is_visitor(bag, 1920)
    assert not is_visitor(far_person, 1920)
    assert is_visitor(person, 1920)
    clip_face = Face(bbox=(200, 100, 445, 380), score=0.71)   # 640 frame
    assert is_visitor(clip_face, 640)
    assert not is_visitor(Face(bbox=(200, 100, 445, 380), score=0.4), 640)
    assert visitors([person, bag], 1920) == [person]
