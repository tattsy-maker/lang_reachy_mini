...
class VoiceIdentity:
    ...
    state and returns an *action* the caller turns into a spoken cue:
        ...
        "overruled"   voice disagrees but the face vouches (T17.2): logged,
                      never spoken; twice -> trusted for the session
    ...
    T17.2: the face is asked *before* the voice speaks up. On 2026-09-05
    one visitor was challenged five times in two sessions while her face
    scored 0.60-0.95 throughout (T15.1 only guarded the "speaker
    changed" outcome; the challenge cue went straight to the model).
    With ``face_vouches()`` set (the session runner's "was the session's
    face seen in the last few seconds"), a mismatch while the face
    vouches is logged and returned as "overruled", never spoken; after
    ``trust_after`` such overrules, or a ``confirm_identity`` that the
    face accepted, ``trust()`` switches voice decisions off for the rest
    of the session (the family: "if they confirmed once or twice, act
    as that person"), and ``absorb()`` folds what was heard into the
    stored print so the next visit starts from a better one.
    """
    ...
    def __init__(self, store, holder, *, learn_after: int = 2,
                 ...
                 face_vouches=None, trust_after: int = 2):
        self.store = store
        self.holder = holder
        # T17.2: ``face_vouches()`` -> bool, bound late by the agent to
        # the session runner; None means "no face to ask" (the voice
        # then behaves as before, the runner's speaker_changed being the
        # only net).
        self.face_vouches = face_vouches
    ...
    def trust(self, reason: str) -> None:
        """No more voice decisions this session (samples still feed the
        running print). Logged once."""
        if not self.trusted:
            self.trusted = True
            self.trust_reason = reason
            logger.info("voice: trusting this session's identity from here "
                        "on (%s); the voice print will not ask again",
                        reason)
        self.rearm()

    def absorb(self, learner, weight: float = 0.5) -> bool:
        """Fold this session's samples into ``learner``'s stored print
        (``weight`` on the new speech). A learner with no print gets the
        session print outright. Returns True when the profile changed."""
        if learner is None or not self.samples:
            return False
        current = self.store.load(learner.id)
        if current is None:
            return False
        session = self.print
        if current.voice_embedding:
            merged = ((1.0 - weight) * np.asarray(current.voice_embedding,
                                                  dtype=np.float32)
                      + weight * session)
            merged = merged / (np.linalg.norm(merged) or 1.0)
        else:
            merged = session
        current.voice_embedding = [float(x) for x in merged]
        self.store.save(current)
        learner.voice_embedding = current.voice_embedding
        logger.info("voice: %s's stored print updated from %d samples "
                    "heard this session", learner.id, len(self.samples))
        return True

    def _face_vouches(self) -> bool:
        if self.face_vouches is None:
            return False
        try:
            return bool(self.face_vouches())
        except Exception as exc:                                # noqa: BLE001
            logger.warning("voice: face_vouches failed: %s", exc)
            return False

    def _overrule(self, learner, score: float) -> str:
        self.overruled += 1
        self.consecutive_mismatches = 0
        logger.info("voice: %s does not sound like themselves (score %.3f) "
                    "but their face is in front of the robot; not asking "
                    "(%d of %d before trusting)", learner.id, score,
                    self.overruled, self.trust_after)
        if self.overruled >= self.trust_after:
            self.trust("the face vouched %d times" % self.overruled)
        return "overruled"
        ...
        if learner is not None:
            ...
            if not stored:
                if len(self.samples) >= self.learn_after:
                    ...
                    return "learned"
                return None
            if self.downgraded or self.changed or self.trusted:
                return None
            if short:
            ...
            if score < self.reject:
                ...
                if self._face_vouches():
                    # T17.2: the face is there and it is them; the
                    # voice keeps its doubt to the log.
                    return self._overrule(learner, score)
                if self.verified:
