...
(a proportional controller), and brings the base round under the head
when the neck is far enough round to look strained. ``head_yaw`` is the
gaze in the world frame (the vendor turns the base under a world-fixed
head), so the base following never moves the gaze.

Rules that keep it from twitching or fighting the other motion sources:

* **Dead-band and rate limit.** Offsets under ``dead_band`` (4 deg) are
  ignored and commands are at least ``min_interval`` (0.8 s) apart, with
  a step cap (12 deg) and moves that take most of the interval, so a
...
class FaceTracker:
    ...
    def __init__(self, robot, *, embodiment=None, gain: float = 0.65,
                 dead_band: float = math.radians(4.0),
                 min_interval: float = 0.8,
                 max_step: float = math.radians(12.0),
        ...
        # 2026-09-23, "keep the person in the centre of its gaze": that
        # left a face up to 6 deg (~100 px) off-centre for good and needed
        # a dozen seconds to catch a visitor who sat down at the edge.
        # Gain 0.65, a 4 deg dead-band and 12 deg steps close the gap in
        # two or three moves; the interval and move length that cured the
        # twitch stay.
        self.robot = robot
    ...
    def observe(self, bbox, width: float, height: float,
        ...
        if move_yaw:
            # head_yaw is the gaze in the WORLD frame: the vendor solves
            # the head pose in the world and turns the base underneath it
            # (2026-09-23). Until then this handed the angle to the body
            # and "recentred" the head to 0 -- which swung the gaze
            # straight back, so a visitor off to one side got a head that
            # climbed to 35 deg and snapped back every ~4.5 s for a whole
            # lesson, and the body estimate ratcheted to its 160 deg clamp.
            step = _clamp(self.gain * yaw_off, self.max_step)
            self._yaw = _clamp(self._yaw + step, HEAD_YAW_LIMIT)
            cmd["head_yaw"] = self._yaw
            if abs(self._yaw - self._body) > self.handoff:
                # The neck has done its share: bring the base round under
                # the head. The gaze does not move, so this is a turn to
                # face the visitor, not a correction.
                self._body = _clamp(self._yaw, BODY_YAW_LIMIT)
...
class TrackingLoop:
    ...
    async def run(self) -> None:
        ...
        while True:
            ...
            if face is not None and recognize.is_visitor(face, frame.shape[1]):
                h, w = frame.shape[:2]
