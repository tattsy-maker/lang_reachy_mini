...
# Cloud mode: this long with no live Gemini connection and the watchdog
# ends the agent (exit code EXIT_BRAIN_LOST) for the supervisor to restart.
# Longer than a go-away reconnect or a visitor reset (seconds each).
GEMINI_DEAD_SECS = 45.0
EXIT_BRAIN_LOST = 75
...
and never speak a tool's name or anything that looks like code or a function \
call: tools are called, not said. If someone asks you to speak louder or \
quieter, call set_volume.
...
def build_tools(robot: RobotLink, tracker: FaceTracker | None = None,
                embodiment=None, budget: SightseeingBudget | None = None) -> list:
    ...
    async def move_head(params):
        if (busy := dance_in_progress()):
            ...
            return
        if budget is not None and (over := budget.allow("move_head")):
            logger.info("move_head: skipped, sightseeing budget spent")
            await params.result_callback(over)
            return
        a = params.arguments
        dofs = {}
    ...
    async def turn_body(params):
        if (busy := dance_in_progress()):
            await params.result_callback(busy)
            return
        if budget is not None and (over := budget.allow("turn_body")):
            logger.info("turn_body: skipped, sightseeing budget spent")
            await params.result_callback(over)
            return
        a = params.arguments
    ...
    async def perform(params):
        ...
        if budget is not None and budget.in_lesson is not None \
                and budget.in_lesson() and passes > 1:
            # 2026-09-23: "the dance was a bit too long" -- 27 s and then
            # 32 s of dancing inside a five-minute lesson. One pass (a
            # few seconds) while someone is being tutored.
    ...
    return [
        FunctionSchema(
            name="move_head",
            description="Point the head. Positive yaw turns the head to "
                        "your own left, which is the visitor's RIGHT as they "
                        "face you; negative yaw is the visitor's left. "
                        "Positive pitch tips the chin down; roll tilts "
                        "ear-to-shoulder. Range is about 50 degrees of yaw "
                        "and 25 of pitch or roll.",
            properties={
        ...
        FunctionSchema(
            name="turn_body",
            description="Rotate the whole robot on its base. Positive is "
                        "your own left, the visitor's right as they face "
                        "you. Up to about 160 degrees either way.",
        ...
        FunctionSchema(
            ...
            description="Play a recorded move: a dance, a spin, or an "
                        "emotion. Use when the visitor asks you to dance, "
                        "spin, or show what you can do. During a lesson a "
                        "move is a short treat they asked for, never your "
                        "own idea mid-exercise, and it plays once. Moves: "
...
def build_look_tool(hub, speech: str, task_ref: dict, gemini_ref: dict,
                    save_dir=None, budget: SightseeingBudget | None = None
                    ) -> list:
    ...
    async def look(params):
        if budget is not None and (over := budget.allow("look")):
            logger.info("look: skipped, sightseeing budget spent")
            await params.result_callback(over)
            return
        got = grab()
        if got is None:
        ...
        # 2026-09-23: on one frame showing only the visitor the model said
        # "I see someone else now!", and on a blurred frame of a room at
        # home, "I am seeing the Maker Faire booth!" (the prompt, not the
        # picture). Say what is in the picture, and let the face check,
        # not the picture, say who it is.
        grounding = ("Describe only what is actually in the picture. Do "
                     "not assume where you are: you may be at home, not at "
                     "the faire. Never announce a new or different person "
                     "from a picture")
        runner = task_ref.get("runner")
        if runner is not None and runner.face_vouches():
            grounding += ("; face recognition says the person in front of "
                          "you is still the same visitor")
        grounding += "."
        if speech == "cloud":
    ...
    async def process_frame(self, frame, direction):
        ...
        if isinstance(frame, LLMTextFrame):
            ...
            if text:
                ...
                # T17.10: the pace has a number, per reply.
                if isinstance(frame, BotStoppedSpeakingFrame) \
                        and self._speaking_since is not None:
                    line = pace_line(text, time.monotonic() - self._speaking_since)
                    if line:
                        logger.info(line)
            if isinstance(frame, BotStoppedSpeakingFrame):
...
class HeardLogger(FrameProcessor):
    """One ``heard: ...`` line per visitor sentence (T17.7), and the hook
    that catches the closing answer from the transcript (T17.9).

    In cloud mode Gemini's input transcription travels *upstream* as
    TranscriptionFrames, which no downstream logger sees; the 2026-09-05
    log had the visitor's words only as pipecat DEBUG lines. Local mode
    gets the same line from Whisper's frames going downstream.
    """

    def __init__(self, on_heard=None):
        super().__init__(name="HeardLogger")
        ...
        if isinstance(frame, TranscriptionFrame) and frame.text.strip():
            ...
            if self.on_heard is not None:
                try:
                    ...
                    logger.warning("heard hook failed: %s", exc)
        await self.push_frame(frame, direction)
...
emit the tool call in that same turn, alongside anything you say.""".format(
    ...
    # T17.8: look / move_head / turn_body share a budget during a lesson
    # (two calls per two minutes), so a visitor playing "turn your head,
    # any other people?" gets steered back instead of obeyed all day.
    late = {}          # {"task": PipelineTask, "service": Gemini}, filled below
    budget = SightseeingBudget(
        in_lesson=lambda: (late.get("holder") is not None
                           and late["holder"].learner is not None))
    ...
    tools = ((build_tools(robot, tracker, embodiment, budget=budget)
              if robot else [])
             + build_audio_tools())
        ...
        if args.face_source:
            ...
            tools = tools + build_enrollment_tools(
                ...
                session_face=lambda: getattr(late.get("runner"), "_session_face",
                                             None),
                # 2026-09-23: a learner met mid-session gets their own
                # briefing as the standing instruction (session mode).
                rebrief=(lambda why: late["runner"].schedule_rebrief(why)
                         if late.get("runner") is not None else None)
        ...
        if holder is not None:
            ...
            if args.wishes_file:
                feedback_path = os.path.join(
                    os.path.dirname(args.wishes_file), "feedback.md")
            catch_feedback(holder, text, time.monotonic(), path=feedback_path)

    # T17.10: slow the reply audio down (pitch kept) when asked to.
    stretcher = None
    ...
    if args.speech == "cloud":
        ...
        stages.append(HeardLogger(on_heard=heard_hook))   # "heard: ..."
        after = [CloudTranscriptLogger()]  # "said: ...", "pace: ..."
        ...
        pipeline = Pipeline(stages + [
            stt,
            HeardLogger(on_heard=heard_hook),
    ...
    # Watchdog for the unattended booth (systemd, start_booth.sh service
    # mode). Pipecat gives up on Gemini after three failed reconnects with
    # a non-fatal ErrorFrame, and a connect that fails outright (network
    # down) just ends its task -- either way the agent stays up, deaf and
    # mute. Leave instead, through the normal cleanup, so the supervisor
    # restarts the stack once the network is back. The heartbeat file
    # tells the supervisor the event loop itself is still turning.
    watchdog_task = None
    if args.heartbeat or args.speech == "cloud":
        async def watchdog():
            down_since = None
            while True:
                ...
                if args.speech == "cloud":
                    now = time.monotonic()
                    if getattr(gemini, "_ready_for_realtime_input", False):
                        down_since = None
                    elif down_since is None:
                        down_since = now
                    elif now - down_since > GEMINI_DEAD_SECS:
                        logger.error("watchdog: Gemini disconnected for %.0fs; "
                                     "exiting so the booth restarts",
                                     now - down_since)
                        late["fatal"] = True
                        await task.cancel()
                        return
                if args.heartbeat:
                    with contextlib.suppress(OSError):
                        with open(args.heartbeat, "a"):
                            os.utime(args.heartbeat)
    ...
    try:
        ...
        for bg in (session_task, tracking_task, watchdog_task):
            if bg is not None:
                bg.cancel()
        ...
        if robot is not None:
            ...
            await robot.close()
    return EXIT_BRAIN_LOST if late.get("fatal") else 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    g.add_argument("--turn-patience-ms", type=int, default=DEFAULT_PATIENCE_MS,
    ...
    g.add_argument("--heartbeat", default=None, metavar="FILE",
                   help="unattended runs: touch FILE every 5 s while the "
                        "agent is alive, for a supervisor to watch "
                        "(start_booth.sh service mode)")
