...
PROFILE_KEYS = ("name", "target_language", "level", "embedding",
                ...
                "voice_embedding", "native_language", "explain_in",
                "corrections")
...
# ``explain_in`` / ``corrections`` (T17.4/T17.5/T17.7): the language
# policy for explanations (native / target / both) and how the learner
# wants to be corrected (every / blocking / end), both asked at intake.
OPTIONAL_KEYS = {"goal": "conversation", "goal_note": "",
                 "voice_embedding": None, "native_language": "en",
                 "explain_in": "native", "corrections": "every"}
...
class Learner:
    ...
    explain_in: str = "native"
    corrections: str = "every"
...
class LearnerStore:
    ...
    def create(self, name: str, target_language: str, *,
               ...
               voice_embedding: list[float] | None = None,
               native_language: str = "en", explain_in: str = "native",
               corrections: str = "every") -> Learner:
        ...
        if explain_in not in EXPLAIN_IN:
            raise ValueError(f"explain_in must be one of {EXPLAIN_IN}, "
                             f"got {explain_in!r}")
        if corrections not in CORRECTIONS:
            raise ValueError(f"corrections must be one of {CORRECTIONS}, "
                             f"got {corrections!r}")
        base = _slugify(name)
        learner_id, n = base, 1
        ...
        learner = Learner(id=learner_id, name=name,
                          ...
                          native_language=(str(native_language or "en")
                                           .strip().lower() or "en"),
                          explain_in=explain_in, corrections=corrections)
    ...
    def load(self, learner_id: str) -> Learner | None:
        ...
        try:
            ...
            if fields["explain_in"] not in EXPLAIN_IN:
                fields["explain_in"] = "native"
            if fields["corrections"] not in CORRECTIONS:
                fields["corrections"] = "every"
